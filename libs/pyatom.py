...
def _make_text_block(name, content, content_type=None):
    ...
    if content_type == 'xhtml':
        return '<%s type="xhtml"><div xmlns="%s">%s</div></%s>\n' % \
...
class AtomFeed(object):
    ...
    def generate(self):
        ...
        if not self.author:
            if False in [bool(e.author) for e in self.entries]:
                self.author = ({'name': 'unbekannter Autor'},)
        ...
        if generator_name or generator_url or generator_version:
            tmp = ['  <generator']
            if generator_url:
                tmp.append(' uri="%s"' % escape(generator_url, True))
            if generator_version:
                tmp.append(' version="%s"' % escape(generator_version, True))
            tmp.append('>%s</generator>\n' % escape(generator_name))
            yield ''.join(tmp)
    ...
    def generate(self):
        ...
        if self.summary:
            yield '  ' + _make_text_block('summary', self.summary,
                                           self.summary_type)
        if self.content:
            yield '  ' + _make_text_block('content', self.content,
                                           self.content_type)
        yield '</entry>\n'
